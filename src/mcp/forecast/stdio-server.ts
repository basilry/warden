...
import { dispatchUnknownForecastToolCall } from "./tools.ts";
...
function handleLine(line: string): void {
  ...
  try {
    ...
    if (typeof toolName !== "string") {
      throw new Error("Forecast MCP tools/call requires params.name.");
    }
    const result = dispatchUnknownForecastToolCall(toolName, request.params?.arguments);
