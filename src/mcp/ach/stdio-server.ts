...
import { dispatchUnknownAchToolCall } from "./tools.ts";
...
function handleLine(line: string): void {
  ...
  try {
    ...
    if (typeof toolName !== "string") {
      throw new Error("ACH MCP tools/call requires params.name.");
    }
    const result = dispatchUnknownAchToolCall(toolName, request.params?.arguments);
