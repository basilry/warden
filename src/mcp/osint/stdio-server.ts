...
import { dispatchUnknownOsintToolCall } from "./tools.ts";
...
async function handleLine(line: string): Promise<void> {
  ...
  try {
    ...
    if (typeof toolName !== "string") {
      throw new Error("OSINT MCP tools/call requires params.name.");
    }
    const result = await dispatchUnknownOsintToolCall(toolName, request.params?.arguments);
